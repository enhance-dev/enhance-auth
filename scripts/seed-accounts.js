...
async function main() {
  await db.set({
    ...
    lastname: 'Doe',
    roles: ['admin', 'member'],
    authenticationConfig: { mfa: { type: 'sms', enabled: true, phone: process.env.TWILIO_TEST_PHONE } },
  ...
  await db.set({
    ...
    roles: ['member'],
    authConfig: { mfa: { enabled: true, type: 'sms', phone: process.env.TWILIO_TEST_PHONE } },
